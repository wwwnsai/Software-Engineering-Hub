...
class Locker_date(persistent.Persistent):
    def __init__(self, date, lockers, status = True) -> None:
        self.date = date
    ...
    def showInfo(self):
        return {
            "reserveDate": self.date,
            "status": self.status,
            "lockers": {id: locker.toJSON() for id, locker in self.lockers.items()}
